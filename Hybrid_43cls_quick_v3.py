...
class ImprovedHybridNet(nn.Module):
    def __init__(self, num_features=12, num_classes=43):
        super(ImprovedHybridNet, self).__init__()
    ...
    def forward(self, x):
        ...
        features = features * attention_weights + identity
        
        # 分类
        return self.classifier(features)

def train_model_with_scheduler(model, train_loader, val_loader, criterion, optimizer, 
                             num_epochs=10, device='cpu', start_epoch=0, save_model="best_model.pth"):
    """Enhanced training function with learning rate scheduling and mixed precision training"""
...
def main():
    ...
    model = ImprovedHybridNet(num_features=num_features, num_classes=43).to(device)
    criterion = nn.CrossEntropyLoss()
    ...
    model = train_model_with_scheduler(
