...
def quantum_encoding(data: np.ndarray) -> np.ndarray:
    ...
    for i in range(min(len(data), 2**num_qubits)):
        if abs(data[i]) > 1e-10:
            ...
            if num_qubits > 1:
                qr.mcry(theta=angle,
                       q_controls=[qr.qubits[j] for j in range(num_qubits-1)],
                       q_target=qr.qubits[-1])
            else:
                qr.ry(angle, 0)
            for j, bit in enumerate(binary):
                if bit == '1':
    ...
    # Get statevector and convert to features
    statevector = result.get_counts()
    
    # Convert counts to normalized features
    features = np.zeros(num_qubits)
    total_shots = sum(statevector.values())
    for state, count in statevector.items():
        for i, bit in enumerate(state.split()[0]):
...
def main():
    ...
    print("Processing training data...")
    train_quantum_features = []
    for path in tqdm(train_paths):
        classical_data = preprocess_image(path)
        quantum_features = quantum_encoding(classical_data)
        train_quantum_features.append(quantum_features)
    
    train_quantum_features = np.array(train_quantum_features)
    
    # Split data
    x_train, x_val, y_train, y_val = train_test_split(
        train_quantum_features, train_labels, test_size=0.2, random_state=42
    )
    ...
    for path in tqdm(test_paths):
        classical_data = preprocess_image(path)
        quantum_features = quantum_encoding(classical_data)
        test_quantum_features.append(quantum_features)
    
    test_quantum_features = np.array(test_quantum_features)
    test_labels = np.array(test_labels)
    
    test_dataset = ImageDataset(test_quantum_features, test_labels)
