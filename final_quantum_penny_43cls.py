...
def QuantumModel():
    model = keras.models.Sequential([
        # 量子编码层已经将28x28x1转换为14x14x4的输出
        # 这里直接指定输入shape为量子编码后的维度
        keras.layers.Input(shape=(14, 14, 4)),
        # 不需要Conv2D因为量子编码已经替代了它
        keras.layers.MaxPooling2D((2,2), padding='same'),
        keras.layers.Flatten(),
        keras.layers.Dense(128, activation='relu'),
        keras.layers.BatchNormalization(),
...
def evaluate_model(model, x_test, y_test, model_type, timestamp):
    ...
    precision, recall, f1, support = precision_recall_fscore_support(
        y_test, 
        y_pred_classes,
        zero_division=0  # 添加这个参数
    )
    
    # 在 classification_report 中也添加
...
def main():
    ...
    if PREPROCESS:
        # Process and save quantum features
