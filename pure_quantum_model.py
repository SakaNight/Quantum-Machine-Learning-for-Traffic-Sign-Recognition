# /opt/anaconda3/bin/python complete_quantum_model.py
import numpy as np
import pandas as pd
...
# Add missing preprocess_image function
def quantum_encode_data(data: np.ndarray) -> np.ndarray:
    """Apply quantum encoding to the input data with improved stability"""
    num_qubits = 12
    ...
    # Use simpler encoding scheme
    for i in range(num_qubits):
        # Scale the data to avoid numerical instability
        angle = np.clip(data[i] * np.pi, -np.pi/2, np.pi/2)
    ...
    # Simulate circuit with increased precision
...
def preprocess_and_encode_image(image_path: str, size: tuple = (32, 32), pca=None) -> np.ndarray:
    """Preprocess image, apply PCA, and then quantum encode"""
    try:
        # Regular preprocessing
        classical_features = preprocess_image(image_path, size, pca)
        
        # Apply quantum encoding
        quantum_features = quantum_encode_data(classical_features)
...
def prepare_quantum_batch(df: pd.DataFrame, pca=None, batch_size: int = 32) -> Tuple[List[np.ndarray], np.ndarray]:
    """Prepare a batch of images with quantum encoding"""
    processed_images = []
    for i in range(0, len(df), batch_size):
        batch_df = df.iloc[i:i+batch_size]
        batch_images = [preprocess_and_encode_image(path, pca=pca) for path in batch_df['Path']]
...
def main():
    ...
    # Get balanced subset from classes 1 and 2
    ...
    print("Fitting PCA...")
    n_components = min(12, len(train_df) - 1)
    pca = PCA(n_components=n_components)
    pca.fit(init_images)
    print(f"PCA with {n_components} components, explained variance ratio:", pca.explained_variance_ratio_.cumsum()[-1])
    
    # Prepare training data with quantum encoding
    print("Preparing quantum encoded training data...")
    train_images, train_labels = prepare_quantum_batch(train_df, pca=pca, batch_size=32)
    ...
    print("Quantum encoded input shape:", x_train.shape)
    ...
    for epoch in range(num_epochs):
        ...
        if val_accuracy > best_val_acc:
            ...
            if hasattr(classifier, '_fit_result') and hasattr(classifier._fit_result, 'x'):
                best_weights = classifier._fit_result.x.tolist()
                save_weights(best_weights)
                print("Saved new best model weights")
    
    # Test predictions using best weights
    print("\nEvaluating on test set...")
    test_images, test_labels = prepare_quantum_batch(test_df, pca=pca, batch_size=32)
    test_images = np.array(test_images)
    test_labels = np.where(np.array(test_labels) == 1, 1, -1)  # Convert classes 1 and 2 to 1 and -1
